...
import {
  ...
  updateQuantity,
  removeAllFromCart,
} from "../../redux/action/cartActions.js";
import { useSelector, useDispatch } from "react-redux";
import { useNavigate } from "react-router-dom";
import "./CartPage.css";
import cartPage from "../../img/cartPage.png";
const CartPage = () => {
  const cart = useSelector((state) => state.cart.items);
  const dispatch = useDispatch();
  const navigate = useNavigate();
  ...
  return (
    <div className="cart-container">
      <div className="cart-header">
        ...
        {/* <button className="remove-all-btn" onClick={handleRemoveAllFromCart}>נקה לגמרי עגלה</button> */}
      </div>
      ...
      <div className="cart-content">
              ...
              {cart.map((item) => (
                <tr key={item._id}>
                  ...
                  <td>
                    <ul>
                      <li>מק"ט: {item._id}</li>
                      <li>וואט: {item.watt.join(", ")}</li>
                      <li>צבע: {item.colors.join(", ")}</li>
                    </ul>
                  </td>
                  <td>
                    <button
        ...
        <div className="image-container">
          <img src={cartPage} alt="תמונה" />
        </div>
      </div>
      <div className="sendeamil">
        {/* <h3>סה"כ מוצרים: {cart.reduce((acc, item) => acc + item.quantity, 0)}</h3> */}
        <input
          type="email"
          placeholder="הזן כתובת מייל לקבלת ההזמנה"
